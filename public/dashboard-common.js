...
function getWishlist() {
    try {
        const value = JSON.parse(localStorage.getItem(DASHBOARD_STORAGE_KEYS.WISHLIST) || "[]");
...
function getReadNotificationIds() {
    try {
        const value = JSON.parse(localStorage.getItem(DASHBOARD_STORAGE_KEYS.NOTIFICATION_READ) || "[]");
...
function setReadNotificationIds(ids) {
    localStorage.setItem(DASHBOARD_STORAGE_KEYS.NOTIFICATION_READ, JSON.stringify(ids));
}
...
function renderNotificationCenter(containerId, badgeId, notifications, onReadChange) {
    const container = document.getElementById(containerId);
    const badge = document.getElementById(badgeId);
    if (!container || !badge) return;

    const readIds = getReadNotificationIds();
    const unreadCount = notifications.filter(item => !readIds.includes(String(item.id))).length;
    ...
    container.innerHTML = notifications.map(notification => {
        ...
        return `
            <div class="notification-item ${isRead ? "read" : "unread"}" data-notification-id="${escapeHTML(notification.id)}">
                ...
                <div class="flex-grow-1">
                    ...
                    <p class="small text-muted mb-1 mt-1">${escapeHTML(notification.message)}</p>
                    <small class="text-muted">${escapeHTML(formatDateTime(notification.time))}</small>
                </div>
            </div>
        `;
    }).join("");
    ...
    container.querySelectorAll(".notification-item.unread").forEach(item => {
        item.addEventListener("click", () => {
            ...
            renderNotificationCenter(containerId, badgeId, notifications, onReadChange);
            onReadChange?.();
        });
    });
}
...
function getUIPreferences() {
    try {
        const value = JSON.parse(localStorage.getItem("agribidUIPreferences") || "{}");
        return { theme: value.theme || "light", compactMode: Boolean(value.compactMode) };
    } catch (error) {
...
function saveUIPreferences(preferences) {
    localStorage.setItem("agribidUIPreferences", JSON.stringify(preferences));
}
...
function applyTheme() {
    ...
    document.querySelectorAll("[data-theme-toggle]").forEach(button => {
        button.textContent = preferences.theme === "dark" ? "☀ Light Mode" : "🌙 Dark Mode";
        button.setAttribute("aria-label", preferences.theme === "dark" ? "Switch to light mode" : "Switch to dark mode");
    });
}
...
function getRecentlyViewedIds() {
    try {
        const value = JSON.parse(localStorage.getItem("agribidRecentlyViewedAuctions") || "[]");
        return Array.isArray(value) ? value.map(Number) : [];
    } catch (error) {
        return [];
    }
...
function getRecentlyViewedFromAuctions(auctions) {
    ...
    return ids.map(id => auctions.find(auction => Number(auction.id) === id)).filter(Boolean);
}

function getBidCountForAuction(bids, auctionId) {
    return bids.filter(bid => Number(bid.auctionId) === Number(auctionId)).length;
}

function renderSimpleBarChart(containerId, items, valueFormatter = value => value) {
    const container = document.getElementById(containerId);
    if (!container) return;
    ...
    container.innerHTML = items.length ? items.map(item => `
        <div class="chart-row">
            ...
            <div class="chart-track"><div class="chart-bar" style="width:${Math.max(4, (Number(item.value) / maxValue) * 100)}%"></div></div>
        </div>
    `).join("") : '<p class="text-muted text-center py-4 mb-0">Not enough data for a chart yet.</p>';
}
